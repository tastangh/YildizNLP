...
log_file_path = os.path.join(log_dir, f'outputAToQ_{timestamp}.log')
...
# Load CSV file
question_answer_path = 'data/results/sampled_question_answer_for_answer.csv'
try:
...
# Function to process a model for finding questions based on answers
def process_model(model_name):
    try:
        ...
        similarities = cosine_similarity(answer_embeddings, question_embeddings)  # Note the switch in order
        angles = np.arccos(np.clip(similarities, -1, 1))

        # Find indices of top 5 questions
        top_5_indices = np.argsort(angles, axis=1)[:, :5]

        # Top 1 accuracy (minimum angular distance)
        top_1_indices = np.argmin(angles, axis=1)

        # Determine true questions (mapping answers back to questions)
        true_questions = questions

        # Calculate accuracy metrics
        top_1_correct = np.sum(np.array(true_questions)[top_1_indices] == true_questions)
        top_5_correct = np.sum([1 if true_questions[i] in np.array(true_questions)[top_5_indices[i]] else 0 for i in range(len(top_5_indices))])

        top_1_accuracy = (top_1_correct / len(answers)) * 100  # Changed denominator to len(answers)
        top_5_accuracy = (top_5_correct / len(answers)) * 100  # Changed denominator to len(answers)
        ...
        all_embeddings = torch.cat((answer_embeddings, question_embeddings), dim=0)
        tsne_results = tsne.fit_transform(all_embeddings)

        # Visualization
        plt.figure(figsize=(10, 8))
        plt.scatter(tsne_results[:len(answers), 0], tsne_results[:len(answers), 1], label='Answers', color='red', alpha=0.5)
        plt.scatter(tsne_results[len(answers):, 0], tsne_results[len(answers):, 1], label='Questions', color='blue', alpha=0.5)
